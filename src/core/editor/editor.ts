...
import { EditorEventArgs, NodeMetaData } from '@/core/type';
export class Editor extends EventBus<EditorEventArgs> {
  ...
  constructor(context: AppContext, graph: Graph) {
    ...
    this.graph = graph;
    this.onGraphCreated({ graph });
  }

  public initGraphPlugin() {
  ...
  public onGraphCreated({graph}: { graph: Graph }) {
    this.graph = graph;
    if (!this.graph) return;
    this.initGraphPlugin();

    this.graph.on('node:selected', (element) => {
      this.emit('ELEMENT_SELECTED', element);
    });

    this.graph.on('node:changed', ({ node }: { node: any }) => {
      this.appContext.project.updateNode(node.store.data)
    });
